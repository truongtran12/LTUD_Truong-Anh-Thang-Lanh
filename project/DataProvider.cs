...
namespace project
{
    public class DataProvider
    {
        ...
        public DataProvider()
        {
            try
            {
                connection.ConnectionString = @"Data Source=MAY39;Initial Catalog=QL_QUANCAFE;Integrated Security=True";
                connection.Open();
            }
            catch { }
        }
        ...
        public void sua_sl_mon(string NAMET, string NAMEF, int count)
        {
            SqlCommand command = new SqlCommand
                ();
            command.Connection = connection;
        ...
        public void sp_sua_total(string NAMET, float total)
        {
            SqlCommand command = new SqlCommand();
            command.Connection = connection;
            command.CommandType = CommandType.StoredProcedure;
            command.CommandText = "sp_sua_total";
            command.Parameters.AddWithValue("@NAMET", SqlDbType.NVarChar).Value = NAMET;
            command.Parameters.AddWithValue("@total", SqlDbType.Int).Value = total;
            command.ExecuteNonQuery();
            connection.Close();
        }
